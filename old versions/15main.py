...
from telegram import Update, WebAppInfo, ReplyKeyboardRemove, KeyboardButton, \
    ReplyKeyboardMarkup, InlineKeyboardMarkup, InlineKeyboardButton, ChatMember
...
# Uses ChatMemberHandler (MY_CHAT_MEMBER) — the correct way in PTB v20+
...
async def welcome_bot_added(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Only trigger when bot goes from non-member → member/admin
    was_in = old_status in (ChatMember.MEMBER, ChatMember.ADMINISTRATOR, ChatMember.OWNER)
    is_in  = new_status in (ChatMember.MEMBER, ChatMember.ADMINISTRATOR, ChatMember.OWNER)

    if was_in or not is_in:
        return  # Not a fresh add
    ...
    log_event("welcome", result.new_chat_member.user.id, result.new_chat_member.user.username,
              chat.id, chat.title)

    logging.info(f"Bot added to group: {chat.title} ({chat.id})")

    # Welcome message with guide button
    keyboard = InlineKeyboardMarkup([
