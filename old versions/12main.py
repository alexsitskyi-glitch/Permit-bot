...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Если пришёл из группы по кнопке — сразу открываем форму
    ...
    await update.message.reply_text(
        ...
        "❓ Ask me about state restrictions anytime.",
        parse_mode="Markdown"
    )
...
async def permit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Из группы — отправляем кнопку которая ведёт в личку
    ...
    # Из лички — открываем Mini App напрямую
    keyboard = ReplyKeyboardMarkup(
        [[KeyboardButton("📋  Open Permit Form", web_app=WebAppInfo(url=WEBAPP_URL))]],
        resize_keyboard=True,
        one_time_keyboard=True
...
# ── /setup — закреплённая кнопка в группе ────────────────────────────────────
