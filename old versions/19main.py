...
# ── УМНЫЙ /start (РАБОТАЕТ СЕЙЧАС И В ГРУППАХ) ──────────────────────────────────
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ЕСЛИ КОМАНДУ ВЫЗВАЛИ В ГРУППЕ
    if chat.type in ("group", "supergroup"):
        log_event("welcome", user.id, user.username, chat.id, chat.title)
        
        # Выдаем инлайн-кнопки (одна открывает WebApp гид, вторая ведет в личку за пермитом)
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton(text="📖  View Quick Guide", web_app=WebAppInfo(url=WELCOME_URL))],
            [InlineKeyboardButton(text="🚛  GET PERMIT", url=get_permit_url)]
        ])
        
        await update.message.reply_text(
            f"👋 *OS/OW Permit Bot is active in this group!*\n\n"
            f"I help drivers submit oversize/overweight permit requests in seconds.\n\n"
            f"Tap *View Quick Guide* to see how it works 👇",
            parse_mode="Markdown",
            reply_markup=keyboard
        )
        return

    # ЕСЛИ КОМАНДУ ВЫЗВАЛИ В ЛИЧКЕ С ПАРАМЕТРОМ
