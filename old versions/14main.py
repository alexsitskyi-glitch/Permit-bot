...
ADMIN_ID         = int(os.environ.get("ADMIN_ID", "0"))   # your Telegram user ID for /stats
...
def get_stats():
    try:
        with sqlite3.connect(DB) as c:
            ...
            # Top 5 drivers by permit count
            top = c.execute("""
                SELECT user_name, COUNT(*) as n FROM events
                WHERE type='permit' GROUP BY user_id ORDER BY n DESC LIMIT 5
            """).fetchall()

            # Top groups
            top_groups = c.execute("""
                SELECT group_name, COUNT(*) as n FROM events
                WHERE type='permit' AND group_name != '' GROUP BY group_id ORDER BY n DESC LIMIT 5
            """).fetchall()
...
async def stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    top_d = "\n".join([f"  {i+1}. {name} — {n} requests" for i,(name,n) in enumerate(s["top_drivers"])]) or "  No data yet"
    top_g = "\n".join([f"  {i+1}. {name} — {n} requests" for i,(name,n) in enumerate(s["top_groups"])]) or "  No data yet"
...
# ── Welcome when bot added to group ──────────────────────────────────────────

async def welcome_bot_added(update: Update, context: ContextTypes.DEFAULT_TYPE):
    bot = await context.bot.get_me()
    if bot.id not in [m.id for m in update.message.new_chat_members]:
        return

    bot_username = bot.username
    chat = update.message.chat

    log_event("welcome", bot.id, bot.username, chat.id, chat.title)

    get_permit_url = f"https://t.me/{bot_username}?start=permit"

    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("📖  View Quick Guide", web_app=WebAppInfo(url=WELCOME_URL))],
        [InlineKeyboardButton("🚛  GET PERMIT", url=get_permit_url)]
    ])

    await update.message.reply_text(
        "👋 *OS/OW Permit Bot is here!*\n\n"
        "I help drivers submit permit requests to dispatch in seconds.\n\n"
    ...
    try:
        pin_keyboard = InlineKeyboardMarkup([[
            InlineKeyboardButton("🚛  GET PERMIT", url=get_permit_url)
        ]])
...
async def handle_webapp_data(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(
            f"⚠️ Could not reach the permits group.\n"
            f"Make sure the bot is an admin in that group.\n"
            f"Error: {e}"
        )
        return
...
async def ask_dify(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query_text.lower().startswith("/info"):
        ...
        if not query_text:
            await update.message.reply_text("❓ Example: /info curfew Illinois", parse_mode="Markdown")
            return
    ...
    try:
        ...
        await update.message.reply_text("⚠️ Could not reach the AI assistant. Try again later.")
