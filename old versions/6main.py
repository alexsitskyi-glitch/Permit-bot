...
async def ask_dify(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Sends the user's question to the Dify knowledge base.
    Maintains conversation_id per user so the GA document context is preserved
    across the entire session — this is what allows Dify to search the loaded KB.
    """
    query_text = update.message.text.strip()

    # Strip /info prefix if used as command
    ...
    # ── KEY FIX: reuse conversation_id so Dify keeps KB context ──────────────
    # Each Telegram user gets their own persistent Dify conversation thread.
    # Without this, every message starts a new session and the GA document
    # is never searched because the app context is reset.
    user_key = f"dify_conv_{update.effective_user.id}"
    conv_id  = context.bot_data.get(user_key)  # persists across messages
    ...
    try:
        ...
        # Save the conversation_id returned by Dify for the next message
...
def main():
    ...
    # ── KEY FIX 1: only respond to free text in PRIVATE chats ────────────────
    # Previously: MessageHandler(filters.TEXT & ~filters.COMMAND, ask_dify)
    # That handler had no chat-type filter, so it caught every message in
    # every group the bot was added to.
    # Now it only fires in 1-on-1 DMs with the bot.
