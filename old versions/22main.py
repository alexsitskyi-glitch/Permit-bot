...
# ── /testgroup — diagnose connection to permits group ─────────────────────────

async def testgroup(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Send a test message to the permits group to verify bot has access."""
    await update.message.reply_text(f"🔍 Testing connection to permits group {PERMITS_GROUP_ID}...")
    try:
        msg = await context.bot.send_message(
            chat_id=PERMITS_GROUP_ID,
            text=f"✅ Test message from Permit Bot\n"
                 f"Bot can reach this group successfully.\n"
                 f"Requested by: {update.effective_user.full_name}"
        )
        await update.message.reply_text(
            f"✅ Success! Bot can send messages to the permits group.\n"
            f"Group ID: {PERMITS_GROUP_ID}\n"
            f"Message ID: {msg.message_id}"
        )
    except Exception as e:
        await update.message.reply_text(
            f"❌ FAILED to reach permits group!\n\n"
            f"Error: {e}\n\n"
            f"Fix: Make sure the bot is added to the group {PERMITS_GROUP_ID} as admin."
        )
...
def build_permit_text(data, driver_name, origin):
    ...
    weight_lines = f"⚖️  Weight:    {data.get('weight', '—')}"
    if is_ow:
        weight_lines += (
...
async def handle_webapp_data(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await context.bot.send_message(chat_id=PERMITS_GROUP_ID, text=permit_text)
    except Exception as e:
    ...
    await update.message.reply_text(
        "✅  Request submitted!\n\nCopy sent to dispatch:\n\n" + permit_text, reply_markup=ReplyKeyboardRemove()
    )
...
async def main():
    ...
    app.add_handler(CommandHandler("testgroup", testgroup))
