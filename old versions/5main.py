...
# Вы можете вынести этот ключ в .env файл как DIFY_API_KEY="..." или оставить прямо здесь
...
# ── Интеграция с Dify ИИ-Ассистентом ──────────────────────────────────────────

async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Приветственное сообщение для водителя."""
...
async def ask_dify(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Пересылает свободный текст или команду /info в Dify базу знаний."""
    query_text = update.message.text.strip()
    
    # Если запрос пришел через команду /info, убираем саму команду из текста
    ...
    # Показываем статус "печатает...", пока Gemini обрабатывает PDF-документы
    ...
    try:
        # requests.post выполняется в отдельном потоке, чтобы асинхронный бот не блокировался
        response = await asyncio.to_thread(
            requests.post, DIFY_API_URL, json=data, headers=headers, timeout=60
        )
        result = response.json()
        answer = result.get('answer', 'Не удалось получить внятный ответ от базы знаний.')
        await update.message.reply_text(answer)
    except Exception as e:
        logging.error(f"Dify API Error: {e}")
        await update.message.reply_text("⚠️ Ошибка связи с ИИ-ассистентом. Попробуйте позже.")
...
STEP_META = {
    ...
    ROUTE: {
        ...
        "back_fixed": None,   # computed dynamically — see get_back()
    },
}
...
def parse_feet(text: str):
    """Extract the leading feet number from strings like 14, 14'6\", 14.5."""
...
def get_back(state: int, data: dict):
    """Return the back callback_data for a state, handling dynamic cases."""
    if state == ROUTE:
...
def build_keyboard(state: int, data: dict) -> InlineKeyboardMarkup | None:
    ...
    if meta["presets"]:
        if state == ROUTE:
            # Each option on its own row for clarity
...
async def get_route(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handles free-text route input after 'Suggest my own route' is chosen."""
...
async def handle_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("val:"):
        ...
        # "Suggest my own route" → show text prompt, stay in ROUTE state
        if value == "__custom__":
            await query.answer()
...
# ── permit text (shared — group and driver see identical form) ────────────────
...
async def send_permit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 2. Show driver their exact copy
    driver_text = (
...
def main():
    ...
    # Добавляем обработчик стартовой команды
    app.add_handler(CommandHandler("start", start))
    
    # Добавляем твой пошаговый опрос на пермиты
    app.add_handler(conv)
    
    # Интеграция ИИ: обрабатывает команду /info и ЛЮБЫЕ текстовые сообщения вне процесса оформления пермита
    app.add_handler(CommandHandler("info", ask_dify))
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, ask_dify))
